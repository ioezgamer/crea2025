...
Route::middleware(['auth', 'verified'])->group(function () {
    ...
    // Es importante definir las rutas más específicas primero para evitar conflictos.

    // Rutas específicas para 'participante' sin parámetros comodín al final que puedan ser interpretados como modelo.
    ...
    Route::get('/participante', [ParticipanteController::class, 'index'])->name('participante.index'); // Lista de todos los participantes
    
    // *** RUTA PARA CARGAR LUGARES DINÁMICAMENTE ***
    // Esta ruta debe definirse ANTES de cualquier ruta como /participante/{parametro}
    ...
    // Rutas de 'participante' que utilizan parámetros (Route Model Binding o IDs)
    // Estas deben ir DESPUÉS de las rutas más específicas de arriba para /participante/.
    Route::get('/participante/{participante}', [ParticipanteController::class, 'show'])->name('participante.show'); // {participante} es un modelo
    Route::get('/participante/{id}/pdf', [ParticipanteController::class, 'exportPdf'])->name('participante.pdf'); // {id} es un parámetro

    // --- RUTAS DE ASISTENCIA ---
    Route::get('/asistencia', [AsistenciaController::class, 'create'])->name('asistencia.create');
    Route::post('/asistencia', [AsistenciaController::class, 'store'])->name('asistencia.store');
    // La ruta duplicada para asistencia.create ha sido eliminada.
    
    // RUTA para guardar asistencia individual (AJAX)
    Route::post('/asistencia/guardar-individual', [AsistenciaController::class, 'storeIndividual'])->name('asistencia.storeIndividual');
    
    // RUTAS para obtener opciones de filtro dinámicas para asistencia
    ...
    Route::middleware('can:manage-roles')->group(function () {
        ...
        // Rutas de exportación e importación para participantes (plural 'participantes')
