<nav x-data="{ open: false }" 
     class="bg-gray-900 text-white fixed top-0 left-0 right-0 z-50 shadow-lg transition-all duration-300 ease-in-out"
        ...
        <div class="flex items-center justify-between h-14">
            <div class="flex items-center">
                ...
                <div class="hidden md:ml-8 md:flex md:items-center md:space-x-4">
                    ...
                    {{-- El enlace 'tutores_participantes' podría ser redundante si 'tutores' ya cubre esto, o podría ser una vista específica --}}
            ...
            <div class="flex items-center">
                <div class="hidden md:block ml-4 relative">
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button class="flex items-center space-x-2 text-sm font-medium rounded-full {{ request()->routeIs('profile.edit') ? 'text-white bg-indigo-600 hover:bg-indigo-500 px-3 py-1.5' : 'text-gray-300 hover:text-white hover:bg-gray-700 px-3 py-1.5' }} focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white transition-colors duration-150">
                        ...
                        <x-slot name="content">
                            ...
                            <form method="POST" action="{{ route('logout') }}">
                                ...
                                <x-dropdown-link :href="route('logout')"
                                    ...
                                    <svg class="h-4 w-4 mr-2 inline-block" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"></path></svg>
                                    {{ __('Cerrar Sesión') }}
                                </x-dropdown-link>
                            </form>
                        </x-slot>
    ...
    <div x-show="open" 
         ...
         class="md:hidden fixed inset-x-0 top-14 z-40 bg-gray-800 text-white shadow-lg" 
         id="mobile-menu"
         @click.away="open = false"> {/* Cierra el menú si se hace clic fuera */}
        ...
        <div class="pt-4 pb-3 border-t border-gray-700">
            <div class="flex items-center px-5">
                <div class="flex-shrink-0">
                    {{-- Puedes añadir un avatar aquí si lo tienes --}}
                    <svg class="h-8 w-8 rounded-full text-gray-400" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M24 20.993V24H0v-2.996A14.977 14.977 0 0112.004 15c4.904 0 9.26 2.354 11.996 5.993zM16.002 8.999a4 4 0 11-8 0 4 4 0 018 0z" />
                    </svg>
                </div>
                <div class="ml-3">
                    <div class="text-sm font-medium leading-none text-white">{{ Auth::user()->name }}</div>
                    @if(Auth::user()->email)
                    <div class="text-xs font-medium leading-none text-gray-400 mt-1">{{ Auth::user()->email }}</div>
                    @endif
                </div>
                {{-- Podrías añadir un botón de notificaciones aquí si es necesario --}}
            </div>
