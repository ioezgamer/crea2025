document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!configElement) {
        console.error('Elemento de configuración para asistencia no encontrado.');
        return;
    }
    ...
    const OPCIONES_PARTICIPANTES_URL = configElement.dataset.opcionesParticipantesUrl;
    ...
    const initialSelectedPrograma = configElement.dataset.initialSelectedPrograma;
    ...
    function checkFiltersAndEnableButton() {
        ...
        if (allFiltersSelected && tablaAsistenciaContainer.querySelector('table')) {
            reportButtonContainer.classList.remove('hidden');
            const reporteUrl = `${REPORTE_URL_BASE}?programa=${encodeURIComponent(filtroPrograma.value)}&lugar_de_encuentro_del_programa=${encodeURIComponent(filtroLugar.value)}&grado_p=${encodeURIComponent(filtroGrado.value)}&fecha=${encodeURIComponent(filtroFecha.value)}&tipo_asistencia=${encodeURIComponent(filtroTipoAsistencia.value)}`;
    ...
    filtroPrograma.addEventListener('change', function () {
        ...
        fetchOptions(`${OPCIONES_LUGARES_URL}?programa=${encodeURIComponent(programa)}`, filtroLugar, filtroGrado, initialSelectedLugar && this.value === initialSelectedPrograma ? initialSelectedLugar : "", 'Seleccione Lugar...');
    });

    filtroLugar.addEventListener('change', function () {
        const programa = filtroPrograma.value;
        const lugar = this.value;
        fetchOptions(`${OPCIONES_GRADOS_URL}?programa=${encodeURIComponent(programa)}&lugar_de_encuentro_del_programa=${encodeURIComponent(lugar)}`, filtroGrado, null, initialSelectedGrado && this.value === initialSelectedLugar && filtroPrograma.value === initialSelectedPrograma ? initialSelectedGrado : "", 'Seleccione Grado...');
    });
    ...
    function showRowFeedback(row, message, type = 'success') {
        ...
        feedbackEl.className = 'ml-2 text-xs transition-opacity duration-300 ease-in-out save-feedback';
        if (type === 'success') {
        ...
        row.style.backgroundColor = type === 'success' ? 'rgba(209, 250, 229, 0.7)' : 'rgba(254, 226, 226, 0.7)';
    ...
    if (initialSelectedPrograma) {
        fetchOptions(`${OPCIONES_LUGARES_URL}?programa=${encodeURIComponent(initialSelectedPrograma)}`, filtroLugar, filtroGrado, initialSelectedLugar, 'Seleccione Lugar...');
    } else {
